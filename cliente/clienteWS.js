function ClienteWS(){
    ...
    this.servidorWS=function(){
        ...
        this.socket.on("partidaAbandonada",function(data){
			if (data.codigo!=-1){
                console.log(data.nombreACTUAL+" ha abandonado la partida con codigo: "+data.codigoP+"\n"+" Ha ganado "+data.nombreRIVAL)
				iu.mostrarHome();
                iu.mostrarModal(data.nombreACTUAL+" ha abandonado la partida con codigo: "+data.codigoP+"\n"+" Ha ganado "+data.nombreRIVAL);
			}
            else{
                console.log("No se ha podido abandonar la partida");
                iu.mostrarModal(data.nombreACTUAL+" ha intentado abandonar la partida pero no ha podido"); 
            }
		});
